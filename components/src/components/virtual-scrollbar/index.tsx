  ...
  useEffect(() => {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      if (isXMoving) {
        ...
        el.scrollLeft = startScrollLeft + (diff / (el.clientWidth - vbarLength)) * (el.scrollWidth - clientWidth);
      }
      if (isYMoving) {
        const diff = ev.clientY - startY;
        el.scrollTop = startScrollTop + (diff / (el.clientHeight - vbarLength)) * (el.scrollHeight - clientHeight);
      }
    };
    ...
    const init = () => {
      ...
      vbarLength = ((clientHeight / scrollHeight) * clientHeight) / 1.5;
      hbarLength = ((clientWidth / scrollWidth) * clientWidth) / 1.5;
